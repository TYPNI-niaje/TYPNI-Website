...
class EventRegistration {
  ...
  constructor(containerId, eventId) {
    ...
    if (!this.supabase) {
      ...
      this.renderError('Failed to initialize registration system');
      return;
    }
  ...
  async checkRegistrationStatus() {
    try {
      ...
      if (registration) {
        ...
        this.renderAlreadyRegistered(registration.status);
      } else {
  ...
  renderCancelationSuccess() {
    ...
    if (registerAgainButton) {
      registerAgainButton.addEventListener('click', () => this.checkRegistrationStatus());
    }
  }
